...
def rank_scores(difficulty_scores):
    ...
    #Sorts in the descending order, more difficult samples on the top
    train_order_ids = list(reversed(sorted(range(len(train_scores)), key=train_scores.__getitem__)))
    dev_order_ids = list(reversed(sorted(range(len(dev_scores)), key=dev_scores.__getitem__)))
