...
class VocabEntry(object):
    ...
    def from_corpus(corpus, size, freq_cutoff=2):
        ...
        vocab_entry.total = sum(vocab_entry.word_freq.values())
...
if __name__ == '__main__':
    ...
    src_sents = read_corpus(args['--train-src'], source='src', space_tokenize=True)
    #print ("These are the source sentences", len(src_sents))
    
    print('read in target sentences: %s' % args['--train-tgt'])
    tgt_sents = read_corpus(args['--train-tgt'], source='tgt', space_tokenize=True)
    #print ("These are the target sentences", len(tgt_sents))
